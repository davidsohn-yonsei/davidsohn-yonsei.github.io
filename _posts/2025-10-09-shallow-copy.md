...
그리고 이후 추가 발견을 하게 된 게 이 분석이 핵심이었다.

[1,2] * 2 = [1,2,1,2] 다. 

애초 리스트는 중복을 허용한다는 점에서 이미 집합과 완전히 일치하지 않지만,

그걸 고려해도 포함집합을 만드는 것이라기엔 상당히 마음에 안 든다.

그러다가 공집합을 더한다, 즉 x + []을 떠올렸다.
